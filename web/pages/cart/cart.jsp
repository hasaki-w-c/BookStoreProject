		...
		$(function () {
			...
			$(".updateCount").change(function () {
				...
				if ( confirm("您确定要将[" + name + "]商品修改数量为：" + count + "吗？") ) {
					//发起请求给服务器保存修改
					location.href = "cartServlet?action=updateCount&count="
							+ count + "&id=" + id;
				}else {
					//defaultValue 属性是表单项 Dom 对象的属性。它表示默认的 value 值。
	...
	<div id="main">
		...
		<table>
			...
			<c:if test="${not empty sessionScope.cart.items}">
				...
				<span class="cart_span"><a class="createOrder" href="orderServlet?action=createOrder">去结账</a></span>
				</div>
			</c:if>
